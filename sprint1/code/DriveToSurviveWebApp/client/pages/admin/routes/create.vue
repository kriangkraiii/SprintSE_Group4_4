            ...
            <div class="mx-auto max-w-7xl">
                ...
                <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-3">
                        <h1 class="text-2xl font-semibold text-primary">สร้างเส้นทาง (แอดมิน)</h1>
                        <span class="text-sm text-slate-400">กรอกข้อมูลเส้นทาง เลือกผู้ขับ และรถที่ใช้</span>
                    </div>
                </div>

                <!-- Card -->
                <div class="flex w-full overflow-hidden bg-white border border-slate-200 rounded-lg shadow-sm">
                    <main class="flex-1 p-8 space-y-8">
                        <!-- SECTION: ผู้ขับขี่ / รถ (ตำแหน่งอ้างอิงจากหน้า detail > ผู้ขับขี่/รถ) -->
                        <section>
                            <h3 class="mb-3 text-sm font-semibold text-primary">ผู้ขับขี่และรถ</h3>
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div class="relative">
                                    ...
                                    <input v-model.trim="driverQuery" @input="onSearchDrivers" type="text"
                                        ...
                                        class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-cta" />
                                    <div v-if="showDriverList"
                                        ...
                                        <button v-for="u in driverResults" :key="u.id" type="button"
                                            ...
                                            class="w-full px-3 py-2 text-left hover:bg-cta-light">
                                            <div class="text-sm font-medium text-primary">
                                                {{ u.firstName }} {{ u.lastName }}
                                                <span v-if="u.username" class="text-xs text-slate-400">(@{{ u.username
                                                    }})</span>
                                            </div>
                                ...
                                <div class="relative">
                                    ...
                                    <input v-model.trim="vehicleQuery" @focus="openVehicleList = true"
                                        ...
                                        class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-cta disabled:bg-slate-100" />
                                    <div v-if="openVehicleList && form.driverId"
                                        ...
                                        <button v-for="v in vehicleFiltered" :key="v.id" type="button"
                                            ...
                                            class="w-full px-3 py-2 text-left hover:bg-cta-light">
                                            <div class="text-sm font-medium text-primary">
                                                {{ v.vehicleModel }} <span class="text-xs text-slate-400">({{
                                                    v.vehicleType }})</span>
                                            </div>
                        ...
                        <section>
                            <h3 class="mb-3 text-sm font-semibold text-primary">แผนที่เส้นทาง</h3>
                        ...
                        <section>
                            <h3 class="mb-3 text-sm font-semibold text-primary">เส้นทาง</h3>
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <!-- Start -->
                                <div>
                                    <label class="block mb-1 text-xs font-medium text-slate-500">จุดเริ่มต้น <span
                                            class="text-red-500">*</span></label>
                                    <div class="relative">
                                        <input ref="startInputEl" v-model="startName" type="text"
                                            placeholder="พิมพ์ชื่อสถานที่ หรือใช้ปุ่มปักหมุด"
                                            class="w-full px-3 py-2 pr-10 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-cta" />
                                    ...
                                    <p class="mt-1 text-xs text-slate-400 truncate" v-if="form.startLocation.address">
                                ...
                                <!-- End -->
                                <div>
                                    <label class="block mb-1 text-xs font-medium text-slate-500">จุดปลายทาง <span
                                            class="text-red-500">*</span></label>
                                    <div class="relative">
                                    ...
                                    <p class="mt-1 text-xs text-slate-400 truncate" v-if="form.endLocation.address">
                        ...
                        <section>
                            <h3 class="mb-3 text-sm font-semibold text-primary">จุดแวะระหว่างทาง</h3>
                            <div class="space-y-3">
                                <div v-for="(w, idx) in form.waypoints" :key="idx"
                                    ...
                                    <div class="relative">
                                        ...
                                        <p class="mt-1 text-xs text-slate-400 truncate" v-if="w.address">
                                            {{ w.address }} ({{ w.lat }}, {{ w.lng }})
                                        </p>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <button type="button" @click="removeWaypoint(idx)"
                                            class="px-3 py-2 border border-slate-200 rounded-md hover:bg-slate-50">
                                            ลบ
                                        </button>
                                    </div>
                                </div>
                                ...
                                <button type="button" @click="addWaypoint"
                                    class="px-4 py-2 text-cta-hover border border-cta rounded-md hover:bg-cta-light">
                                    + เพิ่มจุดแวะ
                                </button>
                            </div>
                        </section>
                        ...
                        <section>
                            <h3 class="mb-3 text-sm font-semibold text-primary">เวลา ราคา และความพร้อมที่นั่ง</h3>
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                <div>
                                    <label class="block mb-1 text-xs font-medium text-slate-500">วัน-เวลาออกเดินทาง <span
                                            class="text-red-500">*</span></label>
                                ...
                                <div>
                                    <label class="block mb-1 text-xs font-medium text-slate-500">ราคา / ที่นั่ง (บาท)
                                        <span class="text-red-500">*</span></label>
                                    <input v-model.number="form.pricePerSeat" type="number" min="0"
                                        class="w-full px-3 py-2 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-cta" />
                                </div>
                                <div>
                                    <label class="block mb-1 text-xs font-medium text-slate-500">ที่นั่งคงเหลือ <span
                                            class="text-red-500">*</span></label>
                        ...
                        <!-- SECTION: เงื่อนไข -->
                        <section>
                            <h3 class="mb-3 text-sm font-semibold text-primary">เงื่อนไขการเดินทาง</h3>
                        ...
                        <div class="flex justify-end gap-4 pt-2">
                            <button type="button" @click="handleCancel" :disabled="isSubmitting"
                                class="px-6 py-3 text-primary border border-slate-200 rounded-md hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-gray-400 disabled:opacity-50">
                                ยกเลิก
                            </button>
                            <button type="button" @click="handleCreate" :disabled="!canSubmit || isSubmitting"
                                class="flex items-center px-6 py-3 font-medium text-white bg-cta rounded-md hover:bg-cta-hover focus:outline-none focus:ring-2 focus:ring-cta disabled:bg-blue-400 disabled:cursor-not-allowed">
            ...
            <div v-if="showPicker" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
                ...
                <div class="bg-white rounded-lg w-[95%] max-w-3xl overflow-hidden">
                    <div class="flex items-center justify-between p-4 border-b border-slate-100">
                        ...
                        <button @click="closePicker" class="text-slate-300 hover:text-slate-500">
