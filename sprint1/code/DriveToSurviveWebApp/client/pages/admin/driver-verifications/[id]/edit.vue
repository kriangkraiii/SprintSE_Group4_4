      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex items-center gap-3">
            <h1 class="text-2xl font-semibold text-primary">แก้ไขคำขอยืนยันตัวตน (แอดมิน)</h1>
            <span class="text-sm text-slate-400">ปรับข้อมูล/อัปโหลดภาพ และอัปเดตสถานะคำขอนี้</span>
          </div>
        </div>
            ...
            <form v-else @submit.prevent="handleSubmit" novalidate class="space-y-8">
              ...
              <div class="mb-2">
                <div class="flex items-center mb-4">
                  ...
                  <h2 class="text-xl font-semibold text-primary">
                    ...
                    <span class="px-2 py-1 ml-2 text-xs text-slate-500 align-middle bg-slate-100 rounded-full">
                      ไม่สามารถแก้ไขเจ้าของคำขอได้
                    </span>
                  </h2>
                </div>

                <label class="block mb-2 text-sm font-medium text-primary">ผู้ใช้</label>
                <input
                  :value="dv?.user ? `${dv.user.firstName || ''} ${dv.user.lastName || ''} (${dv.user.email})` : '-'"
                  type="text"
                  class="w-full px-3 py-2 text-primary border border-slate-200 rounded-lg bg-slate-50"
                  disabled
                />
              </div>
              ...
              <div class="relative">
                ...
                <div class="grid gap-6 mb-2 md:grid-cols-2">
                  <!-- preview ปัจจุบัน -->
                  <div class="space-y-4">
                    <h4 class="text-sm font-medium text-primary">ไฟล์ปัจจุบัน:</h4>
                    <div class="relative flex items-center justify-center h-40 p-4 border border-slate-200 rounded-lg upload-zone bg-slate-50">
                      ...
                      <template v-else>
                        <div class="text-sm text-slate-400">ไม่มีรูป</div>
                      </template>
                    </div>
                  </div>
                  ...
                  <!-- อัปโหลดใหม่ -->
                  <div
                    class="relative flex items-center justify-center h-40 p-8 border-2 border-slate-200 border-dashed rounded-lg upload-zone bg-slate-50 hover:bg-slate-100 hover:border-blue-400"
                  >
                    <div class="text-center pointer-events-none">
                      <i class="mb-2 text-3xl text-slate-300 fa-solid fa-cloud-arrow-up"></i>
                      <p class="text-lg text-slate-500">คลิกเพื่ออัปโหลด</p>
                      <p class="text-sm text-slate-400">JPEG, JPG, PNG (ไม่เกิน 5MB)</p>
                    </div>
                    <input
              ...
              <div class="relative">
                <div class="flex items-center mb-4">
                  ...
                  <h2 class="text-xl font-semibold text-primary">ข้อมูลในบัตรขับขี่ประจำตัว</h2>
                </div>

                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    <label class="block mb-2 text-sm font-medium text-primary">เลขที่ใบขับขี่ <span class="text-red-500">*</span></label>
                    <input v-model.trim="form.licenseNumber" type="text"
                      class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-cta" required />
                  </div>

                  <div>
                    <label class="block mb-2 text-sm font-medium text-primary">ชนิดของบัตรขับขี่ <span class="text-red-500">*</span></label>
                  ...
                  <div>
                    <label class="block mb-2 text-sm font-medium text-primary">ชื่อ (บนบัตร) <span class="text-red-500">*</span></label>
                    <input v-model.trim="form.firstNameOnLicense" type="text"
                      class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-cta" required />
                  </div>

                  <div>
                    <label class="block mb-2 text-sm font-medium text-primary">นามสกุล (บนบัตร) <span class="text-red-500">*</span></label>
                    <input v-model.trim="form.lastNameOnLicense" type="text"
                      class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-cta" required />
                  </div>

                  <div>
                    <label class="block mb-2 text-sm font-medium text-primary">วันออกใบขับขี่ <span class="text-red-500">*</span></label>
                    <input v-model="form.licenseIssueDate" type="date"
                      class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-cta" required />
                  </div>

                  <div>
                    <label class="block mb-2 text-sm font-medium text-primary">วันหมดอายุใบขับขี่ <span class="text-red-500">*</span></label>
