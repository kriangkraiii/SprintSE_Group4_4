<template>
    <div>
        <div class="max-w-4xl px-4 py-8 mx-auto sm:px-6 lg:px-8">
            ...
            <div class="p-8 bg-white border border-slate-200 rounded-lg shadow-sm">
                <form @submit.prevent="handleSubmit" id="postRouteForm" novalidate class="space-y-8">
                    <div>
                        <h3 class="pb-2 mb-6 text-xl font-semibold text-primary border-b border-slate-200">
                            ข้อมูลเส้นทาง
                        </h3>
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <label for="startPoint" class="block mb-2 text-sm font-medium text-primary">
                                ...
                                <div class="relative">
                                    <input ref="startInputEl" v-model="form.startPoint" id="startPoint"
                                        ...
                                        required
                                        class="w-full px-4 py-3 pr-10 border border-slate-200 rounded-md focus:ring-cta" />
                            ...
                            <div>
                                <label for="endPoint" class="block mb-2 text-sm font-medium text-primary">
                            ...
                            <div class="space-y-4">
                                <div v-for="(wp, idx) in waypoints" :key="idx" class="relative">
                                    <input :ref="el => waypointInputs[idx] = el" v-model="wp.text" type="text"
                                        ...
                                        class="w-full px-4 py-3 pr-20 border border-slate-200 rounded-md focus:ring-cta" />
                                    <div class="absolute inset-y-0 flex items-center gap-2 my-auto right-2">
                                        <button type="button" class="text-slate-400 hover:text-cta"
                                ...
                                <button type="button" @click="addWaypoint"
                                    class="inline-flex items-center gap-2 px-3 py-2 text-sm text-white bg-cta rounded-md cursor-pointer hover:bg-cta-hover">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <label for="travelDate" class="block mb-2 text-sm font-medium text-primary">
                                    วันที่เดินทาง <span class="text-red-500">*</span>
                                </label>
                                <input v-model="form.date" id="travelDate" name="travelDate" type="date" required
                                    class="w-full px-4 py-3 border border-slate-200 rounded-md focus:ring-cta" />
                            </div>
                            <div>
                                <label for="travelTime" class="block mb-2 text-sm font-medium text-primary">
                            ...
                            <div>
                                <label for="seatCount" class="block mb-2 text-sm font-medium text-primary">
                            ...
                            <div>
                                <label for="pricePerSeat" class="block mb-2 text-sm font-medium text-primary">
                    ...
                    <div>
                        ...
                        <div v-if="vehicles.length > 0" class="grid items-center grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <label for="vehicle" class="block mb-2 text-sm font-medium text-primary">
                            ...
                            <div class="self-end">
                                <button type="button" @click="isModalOpen = true"
                                    class="w-full px-4 py-3 text-sm text-primary transition-colors bg-slate-100 rounded-md cursor-pointer md:w-auto hover:bg-slate-200">
                                    เพิ่ม / จัดการข้อมูลรถยนต์
                                </button>
                            </div>
                        </div>
                        <div v-else class="p-8 bg-white border border-slate-200 shadow-xl rounded-xl">
                            <div
                                class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-[#F2F2F2] p-4 md:px-6 md:py-6 rounded-[8px]">
                                <p class="text-primary text-base md:text-[18px] text-center sm:text-left">
                                    คุณยังไม่มีข้อมูลรถยนต์
                                </p>
                                <button type="button" @click="isModalOpen = true"
                                    class="bg-[#2563EB] hover:bg-cta text-white text-sm md:text-[16px] px-4 py-2 rounded-lg transition-colors whitespace-nowrap">
                                    เพิ่ม / จัดการข้อมูล
                                </button>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea v-model="form.conditions" id="terms" name="terms" rows="4"
                            ...
                            class="w-full px-4 py-3 border border-slate-200 rounded-md resize-none focus:ring-cta"></textarea>
                    </div>

                    <div class="flex justify-end gap-4 pt-6">
                        <button type="button"
                            class="px-6 py-3 text-primary border border-slate-200 rounded-md cursor-pointer hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-cta">
                            ยกเลิก
                        </button>
                        <button type="submit" :disabled="isLoading"
                            class="px-6 py-3 font-medium text-white bg-cta rounded-md cursor-pointer hover:bg-cta-hover focus:outline-none focus:ring-2 focus:ring-cta disabled:bg-gray-400 disabled:cursor-not-allowed">
...
const handleSubmit = async () => {
    ...
    try {
        ...
        const needDriverVerify =
            is403 && (
            /ยืนยันตัวตนผู้ขับ/.test(msg) ||
            /ใบอนุญาตขับขี่/.test(msg) ||
            /ApiError:.*ยืนยันตัวตนผู้ขับ/.test(msg))

        const needIdVerify =
            is403 && /ยืนยันบัตรประชาชน/.test(msg)

        if (needIdVerify) {
            toast.error('จำเป็นต้องยืนยันตัวตน', 'คุณต้องยืนยันบัตรประชาชนก่อนจึงจะสร้างเส้นทางได้')
            setTimeout(() => { navigateTo('/profile') }, 1500)
        } else if (needDriverVerify || is403) {
            toast.error('จำเป็นต้องยืนยันตัวตน', 'คุณต้องยืนยันบัตรประชาชนและใบขับขี่ก่อนจึงจะสร้างเส้นทางได้')
