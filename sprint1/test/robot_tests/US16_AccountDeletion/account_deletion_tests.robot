...
TC-DEL-004 พิมพ์ข้อความยืนยันไม่ตรง ปุ่มถูก Disable
    ...
    # ตรวจว่าปุ่มยืนยัน disabled
    Get Element States    ${BTN_CONFIRM_DELETE}    contains    disabled
    # ล้าง input
...
TC-DEL-002 ลบบัญชีโดยพิมพ์ DELETE ยืนยัน
    ...
    Wait For Elements State    ${DELETE_MODAL_TITLE}    visible
    # พิมพ์ DELETE
    Type Text    ${DELETE_CONFIRM_INPUT}    DELETE
    # ตรวจว่าปุ่มยืนยัน enabled
    Get Element States    ${BTN_CONFIRM_DELETE}    contains    enabled
    # กดยืนยันลบ
