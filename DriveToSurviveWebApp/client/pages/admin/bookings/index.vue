                ...
                <div class="mb-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                    <div class="grid grid-cols-1 gap-3 px-4 py-4 sm:px-6 lg:grid-cols-[repeat(24,minmax(0,1fr))]">
                        ...
                        <div class="lg:col-span-5">
                            <label class="block mb-1 text-xs font-medium text-slate-500">จุดรับ (ใช้ key: name)</label>
                        ...
                        <!-- Dropoff Name (5/24) -->
                        <div class="lg:col-span-5">
                            <label class="block mb-1 text-xs font-medium text-slate-500">จุดส่ง (ใช้ key: name)</label>
                        ...
                        <table class="min-w-full divide-y divide-slate-100">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-xs font-medium text-left text-slate-400 uppercase">จุดรับ →
                                        จุดส่ง</th>
                                    <th class="px-4 py-3 text-xs font-medium text-left text-slate-400 uppercase">
                                        ออกเดินทาง</th>
                                    <th class="px-4 py-3 text-xs font-medium text-left text-slate-400 uppercase">ที่นั่ง
                                    </th>
                            ...
                            <tbody class="bg-white divide-y divide-slate-100">
                                <tr v-for="b in pagedBookings" :key="b.id" class="transition-opacity hover:bg-slate-50">
                                    <!-- Passenger -->
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-3">
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        <div class="text-xs text-slate-400">
                                            ...
                                            <span class="text-slate-300">({{ b.route?.vehicle?.vehicleType || '-'
                                            }})</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-primary">
                                        ...
                                        <div class="text-xs text-slate-400">อัปเดต {{ formatDate(b.route?.updatedAt) }}
                                        </div>
                                    </td>
                                ...
                                <tr v-if="!pagedBookings.length">
                                    <td colspan="8" class="px-4 py-10 text-center text-slate-400">ไม่มีข้อมูลการจอง</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
function statusBadge(s) {
    ...
    if (s === 'CANCELLED') return 'bg-slate-200 text-primary'
...
async function fetchBookings() {
    ...
    try {
        ...
        bookingsAll.value = Array.isArray(res?.data) ? res.data : []
