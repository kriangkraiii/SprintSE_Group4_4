    ...
    <div>
        <div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="p-6 mb-8 bg-white border border-slate-200 rounded-lg shadow-sm">
                <h2 class="mb-6 text-xl font-semibold text-primary">ค้นหาการเดินทาง</h2>
                <form @submit.prevent="handleSearch" class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
                    <!-- จุดเริ่มต้น -->
                    <div>
                        <label class="block mb-2 text-sm font-medium text-primary">จุดเริ่มต้น</label>
                        <div class="relative">
                            <input ref="originInputEl" v-model="searchForm.origin" type="text"
                                placeholder="เช่น กรุงเทพฯ"
                                class="w-full px-3 py-2 pr-10 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-cta focus:border-transparent">
                            <!-- ไอคอนปักหมุด -->
                            <button type="button" @click="openPlacePicker('origin')"
                                class="absolute inset-y-0 my-auto text-slate-400 right-2 hover:text-cta"
                    ...
                    <!-- จุดปลายทาง -->
                    <div>
                        <label class="block mb-2 text-sm font-medium text-primary">จุดปลายทาง</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-primary">วันที่</label>
                        <input v-model="searchForm.date" type="date"
                            class="w-full px-3 py-2 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-cta focus:border-transparent">
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-primary">จำนวนที่นั่ง</label>
                    ...
                    <div class="flex items-end">
                        <div class="flex items-end gap-2">
                            <button type="submit"
                                class="w-full px-5 py-3 text-white transition duration-200 bg-cta rounded-md cursor-pointer hover:bg-cta-hover focus:outline-none focus:ring-2 focus:ring-cta focus:ring-offset-2">
                                ค้นหา
                            </button>
                            <button type="button" @click="resetSearch"
                                class="flex-1 px-5 py-3 font-medium text-primary transition duration-200 bg-slate-200 rounded-md cursor-pointer hover:bg-slate-300 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">
                                รีเซ็ต
                            </button>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-2 ">
                    <div class="bg-white border border-slate-200 rounded-lg shadow-sm">
                        <div class="p-6 border-b border-slate-200">
                            <h3 class="text-lg font-semibold text-primary">ผลการค้นหา ({{ routes.length }} รายการ)</h3>
                        </div>
                        <div v-if="isLoading" class="p-6 text-center text-slate-400">
                            กำลังค้นหาเส้นทาง...
                        </div>
                        <div v-else class="divide-y divide-slate-100">
                            <div v-if="routes.length === 0" class="p-6 text-center text-slate-400">
                                ไม่พบเส้นทางที่ค้นหา
                            </div>
                            <div v-for="route in routes" :key="route.id"
                                class="p-6 transition-all duration-300 cursor-pointer route-card hover:shadow-sm"
                                @click="toggleDetails(route)">
                                <h1 class="mb-6 font-semibold text-center text-primary">
                                    {{ route.originName }} <span class="mx-1 font-semibold text-primary">→</span> {{
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div>
                                                <div class="flex items-center">
                                                    <h4 class="font-semibold text-primary">{{ route.driver.name }}</h4>

                                                    <div v-if="route.driver.isVerified"
                                                        class="relative group ml-1.5 flex items-center">
                                                        <svg class="w-5 h-5 text-cta" viewBox="0 0 24 24"
                                                            fill="currentColor">
                                            ...
                                            <div class="text-right">
                                                <div class="text-lg font-bold text-cta">{{ route.price }} บาท</div>
                                                <div class="text-sm text-slate-500">ต่อที่นั่ง</div>
                                            </div>
                                        </div>
                                        <div class="mt-3">
                                ...
                                <div v-if="selectedRoute && selectedRoute.id === route.id"
                                    class="pt-4 mt-4 duration-300 border-t border-slate-200 animate-in slide-in-from-top">
                                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        <div>
                                            ...
                                            <ul class="space-y-1 text-sm text-slate-500">
                                                <li>
                                                    • จุดเริ่มต้น:
                                                    <span class="font-medium text-primary">{{ route.originName
                                                        }}</span>
                                                ...
                                                <li class="mt-1">
                                                    • จุดปลายทาง:
                                                    <span class="font-medium text-primary">{{ route.destinationName
                                                        }}</span>
                                            ...
                                            <div class="grid grid-cols-3 gap-2 mt-2 ">
                                                <div v-for="(photo, index) in route.photos.slice(0, 3)" :key="index"
                                                    ...
                                                    <img :src="photo" alt="Vehicle photo"
                                                        class="object-cover w-full transition-opacity border border-slate-200 rounded-lg shadow-sm cursor-pointer aspect-video hover:opacity-90">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex justify-end mt-4">
                                        <button @click.stop="openModal(route)" :disabled="route.availableSeats === 0"
                                            class="px-6 py-2 text-white transition duration-200 bg-cta rounded-md cursor-pointer hover:bg-cta-hover disabled:bg-gray-400 disabled:cursor-not-allowed">
                                            จองที่นั่ง
                                        </button>
                                    </div>
                                </div>
                ...
                <div class="modal-content">
                    <template v-if="!bookingPickingTarget">
                        <div class="flex items-center justify-between p-6 border-b border-slate-200">
                            <h3 class="text-xl font-semibold text-primary">ยืนยันการจอง</h3>
                            <button @click="closeModal" class="text-slate-300 hover:text-slate-500">
                    ...
                    <template v-else>
                        <div class="flex items-center justify-between p-4 border-b border-slate-200">
                            ...
                            <button @click="stopBookingPicker" class="text-slate-300 hover:text-slate-500">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="p-0">
                            ...
                            <div class="flex items-center justify-between p-4 border-t border-slate-100">
                                ...
                                <button
                                    class="px-4 py-2 text-white bg-cta rounded-md hover:bg-cta-hover disabled:bg-gray-400 disabled:cursor-not-allowed"
                    ...
                    <div class="p-6">
                        <div class="mb-6">
                            <h4 class="mb-3 font-semibold text-primary">เดินทางกับ</h4>
                            <div class="flex items-center p-3 space-x-3 rounded-lg bg-slate-50">
                                <img :src="bookingRoute.driver.image" :alt="bookingRoute.driver.name"
                                    class="object-cover w-12 h-12 rounded-full">
                                <div>
                                    <!-- <div class="font-medium text-primary">{{ bookingRoute.driver.name }}</div> -->
                                    <div class="flex items-center">
                                        <div class="font-medium text-primary">{{ bookingRoute.driver.name }}</div>

                                        <div v-if="bookingRoute.driver.isVerified"
                                            class="relative group ml-1.5 flex items-center">
                                            <svg class="w-5 h-5 text-cta" viewBox="0 0 24 24" fill="currentColor">
                                                <path fill-rule="evenodd"
                                                    d="M8.603 3.799A4.49 4.49 0 0112 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 013.498 1.307 4.491 4.491 0 011.307 3.497A4.49 4.49 0 0121.75 12c0 1.357-.6 2.573-1.549 3.397a4.49 4.49 0 01-1.307 3.498 4.491 4.491 0 01-3.497 1.307A4.49 4.49 0 0112 21.75a4.49 4.49 0 01-3.397-1.549 4.49 4.49 0 01-3.498-1.306 4.491 4.491 0 01-1.307-3.498A4.49 4.49 0 012.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 011.307-3.497 4.49 4.49 0 013.497-1.307zm7.007 6.387a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.07-.01l3.5-4.875z"
                                                    clip-rule="evenodd" />
                                            </svg>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span class="ml-2 text-sm text-slate-500">{{ bookingRoute.driver.rating }} ({{
                        ...
                        <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2">
                            <div>
                                <label class="block mb-2 text-sm font-medium text-primary">วันที่เดินทาง</label>
                                <div class="p-3 text-primary rounded-lg bg-slate-50">
                                    {{ bookingRoute.date }}
                                </div>
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-medium text-primary">เวลาออก</label>
                                <div class="p-3 text-primary rounded-lg bg-slate-50">
                        ...
                        <div class="mb-6">
                            <h4 class="mb-3 font-semibold text-primary">เส้นทางการเดินทาง</h4>
                            <div class="flex items-center p-3 space-x-4 rounded-lg bg-slate-50">
                                <div class="flex-1">
                                    <div class="font-medium text-primary">{{ bookingRoute.originName }}</div>
                                    <div class="text-sm text-slate-500">จุดเริ่มต้น</div>
                                </div>
                                <div class="text-cta">
                                ...
                                <div class="flex-1 text-right">
                                    <div class="font-medium text-primary">{{ bookingRoute.destinationName }}</div>
                                    <div class="text-sm text-slate-500">จุดปลายทาง</div>
                                </div>
                            </div>
                        </div>
                        <div class="mb-6 space-y-4">
                            <div>
                                <label
                                    class="block mb-2 text-sm font-medium text-primary">จำนวนที่นั่งที่ต้องการจอง</label>
                            ...
                            <div>
                                <label class="block mb-2 text-sm font-medium text-primary">เลือกจุดขึ้นรถ</label>
                                <div class="relative">
                            ...
                            <div>
                                <label class="block mb-2 text-sm font-medium text-primary">เลือกจุดลงรถ</label>
                                <div class="relative">
                        ...
                        <div class="p-4 mb-6 rounded-lg bg-cta-light">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-primary">ราคาต่อที่นั่ง</span>
                                <span class="font-medium text-primary">{{ bookingRoute.price }} บาท</span>
                            </div>
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-primary">จำนวนที่นั่ง</span>
                                <span class="font-medium text-primary">{{ bookingSeats }} ที่นั่ง</span>
                            </div>
                            <div class="pt-2 mt-2 border-t border-slate-200">
                                <div class="flex items-center justify-between">
                                    <span class="font-semibold text-primary">ยอดรวม</span>
                                    <span class="text-lg font-bold text-cta">{{ bookingTotalPrice }} บาท</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex space-x-4">
                            <button @click="closeModal"
                                class="flex-1 px-4 py-3 font-semibold text-primary transition duration-200 bg-slate-200 rounded-md hover:bg-slate-300">
                                ยกเลิก
                            </button>
                            <button @click="confirmBooking"
                                class="flex-1 px-4 py-3 font-semibold text-white transition duration-200 bg-cta rounded-md hover:bg-cta-hover">
                                ยืนยันการจอง
                            </button>
                        </div>
                    </div>
        ...
        <transition name="modal-fade">
            <div v-if="showPlacePicker" class="modal-overlay" @click.self="closePlacePicker">
                <div class="modal-content">
                    <div class="flex items-center justify-between p-4 border-b border-slate-200">
                        <h3 class="text-lg font-semibold text-primary">เลือก{{ pickingField === 'origin' ?
                            'จุดเริ่มต้น' : 'จุดปลายทาง' }}</h3>
                        <button @click="closePlacePicker" class="text-slate-300 hover:text-slate-500">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
                            </svg>
                    ...
                    <div class="p-4 space-y-3">
                        <div ref="placePickerMapEl" class="w-full border border-slate-100 rounded-md h-80"></div>
                        <div class="text-sm text-primary">
                            <div class="font-medium">ตำแหน่งที่เลือก:</div>
                            <div class="truncate">{{ pickedPlace.name || '— ยังไม่เลือก —' }}</div>
                            <div v-if="pickedPlace.lat && pickedPlace.lng" class="text-slate-400">
                                lat: {{ pickedPlace.lat.toFixed(6) }}, lng: {{ pickedPlace.lng.toFixed(6) }}
                            </div>
                        </div>
                        <div class="flex gap-3 pt-2">
                            <button @click="closePlacePicker"
                                class="flex-1 px-4 py-2 font-semibold text-primary bg-slate-200 rounded-md hover:bg-slate-300">
                                ยกเลิก
                            </button>
                            <button :disabled="!pickedPlace.name" @click="applyPickedPlace"
                                class="flex-1 px-4 py-2 font-semibold text-white bg-cta rounded-md hover:bg-cta-hover disabled:bg-gray-400 disabled:cursor-not-allowed">
                                ใช้ตำแหน่งนี้
                            </button>
                        </div>
                    </div>
...
async function confirmBooking() {
    ...
    try {
        ...
        closeModal();
        toast.success('จองเส้นทางสำเร็จ!', 'การจองของคุณได้รับการยืนยันอัตโนมัติแล้ว');
        ...
        if (is403 && /ยืนยันบัตรประชาชน/.test(msg)) {
            toast.error('ต้องยืนยันตัวตนก่อน', 'คุณต้องยืนยันบัตรประชาชนก่อนจึงจะจองเส้นทางได้');
