<template>
  ...
  >
    <div
      class="relative max-w-[75%] px-4 py-2.5 rounded-2xl shadow-sm"
      :class="bubbleClass"
    >
      <!-- System message -->
      ...
      <div v-if="isOwn && canUnsend && !message.isUnsent" class="absolute -top-2 -left-2">
        <button
          ...
          class="p-1 bg-white rounded-full shadow-md text-slate-400 hover:text-red-500 transition-colors"
          title="ยกเลิกข้อความ"
        >
      ...
      <div v-if="!isOwn && !message.isUnsent && message.type !== 'SYSTEM'" class="absolute -top-2 -right-2">
        <button
          ...
          class="p-1 bg-white rounded-full shadow-md text-slate-400 hover:text-orange-500 transition-colors"
          title="รายงานข้อความ"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
