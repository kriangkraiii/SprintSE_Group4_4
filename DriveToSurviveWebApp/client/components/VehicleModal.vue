    ...
    <transition name="modal-fade">
        <div v-if="show" class="modal-overlay" @click.self="$emit('close')">
            ...
            <div v-if="currentView === 'list'"
                ...
                <div class="flex items-center justify-between p-8 pb-6 border-b border-slate-100">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-xl font-semibold text-primary">จัดการข้อมูลรถยนต์</h2>
                            <p class="text-slate-500 mt-1">เลือกรถยนต์ที่จะใช้ หรือเพิ่มคันใหม่</p>
                        </div>
                    </div>
                    <button @click="$emit('close')"
                        class="text-slate-300 hover:text-slate-500 transition-colors p-2 hover:bg-slate-100 rounded-full">
                ...
                <div class="p-8 space-y-6 overflow-y-auto">
                    <button @click="showFormView('add')"
                        class="w-full border-2 border-dashed border-blue-300 text-cta py-6 rounded-xl hover:bg-cta-light hover:border-blue-400 transition-all duration-200 flex items-center justify-center gap-3 group">
                        <div
                            class="w-8 h-8 bg-cta-light rounded-full flex items-center justify-center group-hover:bg-blue-200 transition-colors">
                            <span class="text-xl font-medium">+</span>
                        </div>
                        <span class="font-medium text-lg">เพิ่มรถยนต์คันใหม่</span>
                    </button>

                    <div v-if="isLoading" class="text-center py-8 text-slate-400">กำลังโหลด...</div>

                    <div v-if="!isLoading && vehicles.length > 0" v-for="vehicle in vehicles" :key="vehicle.id"
                        class="border-2 rounded-xl p-4 relative"
                        :class="vehicle.isDefault ? 'border-cta bg-cta-light' : 'border-slate-200'">
                        <div class="flex items-start gap-4">
                            <img :src="vehicle.photos?.[0] || 'https://placehold.co/100x100/e2e8f0/e2e8f0'"
                                alt="Vehicle Photo" class="w-20 h-20 rounded-lg object-cover bg-slate-200">
                            <div class="flex-1">
                                <p class="font-semibold text-primary">{{ vehicle.vehicleModel }}</p>
                                <p class="text-sm text-slate-500">{{ vehicle.licensePlate }}</p>
                                <p class="text-sm text-slate-400">{{ vehicle.vehicleType }} - {{ vehicle.color }}</p>
                            ...
                            <div class="flex flex-col gap-2">
                                <button @click="showFormView('edit', vehicle)"
                                    class="text-cta text-sm border-2 border-cta px-3 py-1 rounded-lg hover:bg-cta-light transition-all duration-200 font-medium">แก้ไข</button>
                        ...
                        <button v-if="!vehicle.isDefault" @click="handleSetDefault(vehicle.id)"
                            class="mt-3 w-full text-center text-sm text-slate-500 hover:text-cta font-medium">
                            ตั้งเป็นค่าเริ่มต้น
                        </button>
                    </div>

                    <div v-if="!isLoading && vehicles.length === 0" class="text-center py-8">
                        <p class="text-slate-400 text-base">คุณยังไม่มีข้อมูลรถยนต์ที่บันทึกไว้</p>
                    </div>
                </div>

                <div class="flex gap-4 p-8 pt-0 mt-auto border-t border-gray-100">
                    <button @click="$emit('close')"
                        class="flex-1 border-2 border-slate-200 text-primary py-3 px-6 rounded-xl hover:bg-slate-50 transition-colors font-medium">
                        ปิด
                    </button>
                    <button @click="$emit('close')"
            ...
            <div v-if="currentView === 'form'"
                ...
                <div
                    class="flex items-center gap-4 p-8 pb-6 sticky top-0 bg-white border-b border-slate-200 rounded-t-2xl z-10">
                    <button @click="currentView = 'list'" class="text-slate-400 hover:text-primary"><svg class="w-6 h-6"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7">
                            </path>
                        </svg></button>
                    <div>
                        <h2 class="text-xl font-semibold text-primary">{{ formMode === 'add' ? 'เพิ่มรถยนต์คันใหม่' :
                            'แก้ไขข้อมูลรถยนต์' }}</h2>
                        <p class="text-slate-500 mt-1">กรอกรายละเอียดรถยนต์ของคุณให้ครบถ้วน</p>
                    </div>
                </div>
                ...
                <form @submit.prevent="handleFormSubmit" class="p-8 space-y-6 overflow-y-auto">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-primary mb-2">ยี่ห้อและรุ่นรถ</label>
                            <input type="text" v-model="form.vehicleModel" placeholder="เช่น Toyota Camry"
                                class="form-input">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-primary mb-2">หมายเลขทะเบียน</label>
                            <input type="text" v-model="form.licensePlate" placeholder="เช่น กก 1234 ขอนแก่น"
                                class="form-input">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-primary mb-2">ชนิดของรถ</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-primary mb-2">สีรถ</label>
                            <input type="text" v-model="form.color" placeholder="เช่น สีดำ" class="form-input">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-primary mb-2">จำนวนที่นั่ง (ไม่รวมคนขับ)</label>
                        <input type="number" v-model.number="form.seatCapacity" min="1" max="12" placeholder="เช่น 4"
                            class="form-input">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-primary mb-2">สิ่งอำนวยความสะดวก (คั่นด้วยจุลภาค
                            ,)</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-primary mb-2">รูปภาพรถยนต์ (3 รูป)</label>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div v-for="(label, index) in ['รูปด้านหน้า', 'รูปด้านข้าง', 'รูปภายใน']" :key="index">
                                ...
                                <div @click="photoInputs[index]?.click()"
                                    class="image-uploader border-2 border-dashed border-slate-200 rounded-lg flex items-center justify-center text-slate-300 hover:border-blue-400 hover:text-blue-500 transition-colors cursor-pointer bg-slate-50 relative">
                    ...
                    <div class="bg-cta-light border border-blue-200 rounded-xl p-4">
                        <div class="flex items-start gap-3">
                            <input type="checkbox" v-model="form.isDefault" id="isDefault"
                                class="w-5 h-5 text-cta border-2 border-slate-200 rounded focus:ring-cta mt-0.5">
                            <div>
                                <label for="isDefault"
                                    class="text-primary font-medium cursor-pointer">ตั้งเป็นรถยนต์คันหลัก</label>
                                <p class="text-slate-500 text-sm mt-1">รถยนต์นี้จะถูกเลือกโดยอัตโนมัติในครั้งถัดไป</p>
                            </div>
                        </div>
                    </div>
