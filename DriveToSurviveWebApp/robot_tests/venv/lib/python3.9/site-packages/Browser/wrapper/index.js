...
var tryToTransformStringToFunction = (str) => {
  try {
    ...
    logger5.debug(`Failed to transform string to function: ${ignored}`);
    return str;
  }
};
...
async function waitForAlerts(request2, page) {
  ...
  for (let index = 0; index < alertActions.length; index++) {
    ...
    logger7.info(`Waiting for alert with action: ${action}, promptInput: "${promptInput}" and timeout: ${timeout}`);
...
async function _finishContextResponse(context, browserState, traceFile, options) {
  ...
  const response = new import_playwright_pb4.Response.NewContextResponse();
...
function async_logger(propertyKey, propertyDescriptor) {
  ...
  propertyDescriptor.value = async function(...args) {
    try {
      logger10.info(`Start of node method ${propertyKey}`);
      const result = await originalMethod.apply(this, args);
      logger10.info(`End of node method ${propertyKey}`);
      return result;
    } catch (err) {
      logger10.info(`Error of node method  ${propertyKey}`);
      throw err;
    }
  };
    ...
    this.wrapping = (func) => {
      return async (call, callback) => {
        try {
          ...
          logger11.info(`Start of node method ${func.name}`);
          const response = await func(request2, this.getState(call));
          logger11.info(`End of node method ${func.name}`);
          callback(null, response);
        } catch (e) {
          logger11.info(`Error of node method  ${func.name}`);
...
server.bindAsync(`127.0.0.1:${port}`, import_grpc_js2.ServerCredentials.createInsecure(), () => {
  logger12.info(`Listening on ${port}`);
  server.start();
});
